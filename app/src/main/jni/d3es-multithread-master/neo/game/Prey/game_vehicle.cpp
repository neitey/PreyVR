...
void hhVehicle::ProcessPilotInput( const usercmd_t* cmds, const idAngles* viewAngles ) {
	...
	//Lubos BEGIN
	if( viewAngles ) {
		if( game->isVR && pVRClientInfo->vehicleMode && !pVRClientInfo->inMenu && GetPilot() && GetPilot()->IsType(hhPlayer::Type) ) {
			idAngles offset;
			offset.pitch = pVRClientInfo->vehicleOffset[ PITCH ];
			offset.yaw = pVRClientInfo->vehicleOffset[ YAW ];
			offset.roll = pVRClientInfo->vehicleOffset[ ROLL ];
			SetAxis( offset.ToMat3() * viewAngles->ToMat3() );
		} else {
