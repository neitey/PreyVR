...
void idFileSystemLocal::Init(void)
{
	...
	if (fs_cdpath.GetString()[0] == '\0' && Sys_GetPath(PATH_CONFIG, path))
		fs_cdpath.SetString(path);
...
void idFileSystemLocal::StartBackgroundDownloadThread()
{
	if (!backgroundThread.threadHandle) {
		Sys_CreateThread(reinterpret_cast<xthread_t>(BackgroundDownloadThread), NULL, THREAD_NORMAL, backgroundThread, "backgroundDownload", g_threads, &g_thread_count);
