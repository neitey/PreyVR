...
void idImage::GenerateShadowArray( int width, int height, int numSides, textureFilter_t filterParm, textureRepeat_t repeatParm )
{
	...
	switch (filter) {
		case TF_DEFAULT:
			glTexParameterf(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MIN_FILTER, globalImages->textureMinFilter);
			glTexParameterf(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MAG_FILTER, globalImages->textureMaxFilter);
			break;
		case TF_LINEAR:
	...
	//GL_CheckErrors("GenerateShadowMapDepthCubeImage::start");
	glTexImage3D( GL_TEXTURE_2D_ARRAY, 0, internalFormat, scaled_width, scaled_height, numSides, 0, GL_DEPTH_COMPONENT, GL_UNSIGNED_INT, NULL );
