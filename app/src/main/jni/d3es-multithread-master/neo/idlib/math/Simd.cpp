...
void TestDot(void)
{
	...
	for (j = 0; j < 50 && j < COUNT; j++) {
		...
		result = idMath::Fabs(dot1 - dot2) < 1e-4f ? "ok" :  "X";
...
void TestMinMax(void)
{
	...
	result = (v2min == v2min2 && v2max == v2max2) ? "ok" :  "X";
	...
	result = (vmin == vmin2 && vmax == vmax2) ? "ok" :  "X";
...
void TestMemcpy(void)
{
	...
	for (i = 5; i < 8192; i += 31) {
		...
		for (j = 0; j < i; j++) {
			if (test1[j] != test0[j]) {
#ifdef _K_CLANG //k
				idLib::common->Printf("   simd->Memcpy() "   "X\n");
#else
				idLib::common->Printf("   simd->Memcpy() " "X\n");
#endif
				return;
	...
	for (i = 5; i < 8192; i += 31) {
		for (j = -1; j <= 1; j++) {
			...
			for (k = 0; k < i; k++) {
				if (test[k] != (byte)j) {
#ifdef _K_CLANG //k
					idLib::common->Printf("   simd->Memset() "   "X\n");
#else
					idLib::common->Printf("   simd->Memset() " "X\n");
#endif
					return;
...
void TestMatXMultiplyVecX(void)
{
	...
	for (i = 1; i <= 6; i++) {
		...
		result = dst.Compare(tst, MATX_SIMD_EPSILON) ? "ok" :  "X";
...
void TestMatXMultiplyMatX(void)
{
	...
	for (i = 1; i <= 5; i++) {
		...
		result = dst.Compare(tst, MATX_MATX_SIMD_EPSILON) ? "ok" :  "X";
...
void TestMatXLowerTriangularSolve(void)
{
	...
	for (i = 1; i < MATX_LTS_SOLVE_SIZE; i++) {
		...
		result = x.Compare(tst, MATX_LTS_SIMD_EPSILON) ? "ok" :  "X";
...
void TestMatXLDLTFactor(void)
{
	...
	for (i = 1; i < MATX_LDLT_FACTOR_SOLVE_SIZE; i++) {
		...
		result = mat1.Compare(mat2, MATX_LDLT_SIMD_EPSILON) && invDiag1.Compare(invDiag2, MATX_LDLT_SIMD_EPSILON) ? "ok" :  "X";
...
void TestTracePointCull(void)
{
	...
	result = (i >= COUNT && totalOr1 == totalOr2) ? "ok" :  "X";
...
void TestCreateShadowCache(void)
{
	...
	result = (i >= COUNT && numVerts1 == numVerts2) ? "ok" :  "X";
...
void TestSoundUpSampling(void)
{
	...
	for (numSpeakers = 1; numSpeakers <= 2; numSpeakers++) {
		...
		for (kHz = 11025; kHz <= 44100; kHz *= 2) {
			...
			result = (i >= MIXBUFFER_SAMPLES*numSpeakers) ? "ok" :  "X";
	...
	for (numSpeakers = 1; numSpeakers <= 2; numSpeakers++) {
		...
		for (kHz = 11025; kHz <= 44100; kHz *= 2) {
			...
			result = (i >= MIXBUFFER_SAMPLES) ? "ok" :  "X";
...
void TestSoundMixing(void)
{
	...
	result = (i >= MIXBUFFER_SAMPLES*6) ? "ok" :  "X";
