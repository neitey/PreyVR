...
static idList<sysEvent_t> event_list;
...
{
    kbd_polls.Append(kbd_poll_t(key, pressed));

    sysEvent_t res;
    res.evType = SE_KEY;
    res.evValue = key;
    res.evValue2 = pressed ? 1 : 0;
    res.evPtrLength = 0;
    res.evPtr = NULL;
...
extern "C" void Sys_AddMouseMoveEvent(int dx, int dy)
{
    ...
    event_list.Append(sys->GenerateMouseMoveEvent(dx, dy));
}
...
extern "C" void Sys_AddMouseButtonEvent(int button, bool pressed)
{
    switch (button) {
        case 1:
            mouse_polls.Append(mouse_poll_t(M_ACTION1, pressed ? 1 : 0));
            event_list.Append(sys->GenerateMouseButtonEvent(1, pressed));
            break;
        case 2:
            mouse_polls.Append(mouse_poll_t(M_ACTION3, pressed ? 1 : 0));
            event_list.Append(sys->GenerateMouseButtonEvent(3, pressed));
            break;
        case 3:
            mouse_polls.Append(mouse_poll_t(M_ACTION2, pressed ? 1 : 0));
            event_list.Append(sys->GenerateMouseButtonEvent(2, pressed));
            break;
    }
}
...
sysEvent_t Sys_GetEvent() {
    ...
    if (event_list.Num() > eventIndex)
    {
        return event_list[eventIndex++];
    }
